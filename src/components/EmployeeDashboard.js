...
function EmployeeDashboard(props) {
  ...
  const approvedCount = leaveHistory.filter((h) => h.status === "Approved").length;

  const rejectedCount = leaveHistory.filter((h) => h.status === "Rejected").length;
    ...
    <div className="container py-4">
      <div className="mb-4">
        ...
        {/* <p className="text-muted">
          Welcome! {selectedEmployee?.name || "Employee"}
        </p> */}
      </div>

      <div className="alert alert-info mb-4">
        <strong>Welcome </strong> {selectedEmployee?.name || "Employee"}
      </div>
