...
function EmployeeDetails(props) {
  ...
  async function handleDelete() {
    try {
      await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/employees/${employee.id}`, { method: "DELETE" });
      if (props.fetchEmployees) await props.fetchEmployees();
  ...
  async function handleResetLeaves() {
    try {
      await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/employees/${employee.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          leavesLeft: employee.totalLeaves,
          leaveStatus: "",
          rejectionReason: ""
        })
      });
  ...
  async function handleApprove() {
    try {
      await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/leave/employee/${employee.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
